function job = cg_mantis_make_csf_mask
...
job         = cfg_exbranch;
job.tag     = 'mskcsf';
job.name    = 'Threshold segmentation of CSF';
job.val     = {vols phase1Dir phase2Dir};
job.help    = {
    'Segmentation of structural image using thresholding. Drop in' 
    'replacement for watershed, especially for scans with thick slices'
    'and normal ventricles'
    };
job.prog = @cg_mantis_make_csf_mask_run;
job.vout = @vout;
...
cdep = cfg_dep;
cdep(end).sname      = 'Threshold csf seg';
